<?php
	...
	if(isset($_SESSION['detalle']) && count($_SESSION['detalle'])>0){
		...
		//$m->addTo('elis.arcia@gmail.com');
		//$m->addTo(array($email,'Ventas <Refermatventasperu@gmail.com>'));
		$m->addTo(array($email,'Ventas <refermatventasperu@gmail.com>'));
		$m->setFrom('REFERMAT VENTAS <infoperu@refermat.com>');
		$m->setSubject('MENSAJERIA - REFERMAT');
 
	$m->setMessageFromString(" ","".$mensaje_final."<div style='text-align: center;'>
							<table>
							...
							");
		if (!$ses->sendEmail($m)) {
			echo "Mailer Errorx: " . $mail->ErrorInfo;
		} else {
